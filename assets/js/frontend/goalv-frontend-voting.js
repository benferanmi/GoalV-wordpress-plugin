/**
 * GoalV Frontend Voting System
 * Version: 8.2.0
 * 
 * Modular wrapper for existing voting functionality
 * Handles voting for homepage and single match pages
 */
...
(function($) {
    ...
    window.GoalVVoting = {
        ...
        updateSimpleVotingUI: function(matchId, location, results, userVotesByCategory) {
            ...
            if (selectedOptionId) {
                ...
                if ($selectedBtn.length) {
                    ...
                    console.log('Selected option ID on homepage:', selectedOptionId);
                }
            }
        },
        ...
        updateVoteUI: function(matchId, location, results, selectedOptionIds) {
            ...
            console.log('UpdateVoteUI - Match:', matchId, 'Location:', location, 'Selected IDs:', selectedOptionIds);
            ...
            $container.find('.goalv-vote-btn, .goalv-grid-vote-btn').each(function() {
                ...
                if (result) {
                    ...
                    if (selectedOptionIds.includes(optionId)) {
                        ...
                        console.log('Adding selected to option:', optionId);
                    } else {
                        $btn.removeClass('selected').removeAttr('data-selected');
                        console.log('Removing selected from option:', optionId);
                    }
                }
            });
            ...
            $container.find('.goalv-vote-btn').each(function() {
                ...
                if (result) {
                    ...
                    if (selectedOptionIds.includes(optionId)) {
                        ...
                        console.log('Adding selected to detailed option:', optionId);
                    } else {
                        $btn.removeClass('selected').removeAttr('data-selected');
                        $option.removeClass('selected');
                        console.log('Removing selected from detailed option:', optionId);
                    }
                }
            });
        ...
        initStoredVotes: function() {
            ...
            $('.goalv-match-card, .goalv-match-grid-item, .goalv-detailed-voting, .goalv-table-row').each(function() {
                ...
                if (hasGroupedStructure) {
                    ...
                    const storedVotesByCategory = self.getStoredVotesByCategory(matchId, location);
            ...
            $('.goalv-vote-btn, .goalv-grid-vote-btn, .goalv-vote-btn-inline').each(function() {
                ...
                if (Object.keys(storedVotesByCategory).length === 0) {
                    const storedVotes = self.getStoredVotes(matchId, location);
        ...
        updateVoteResults: function() {
            ...
            $('.goalv-match-card, .goalv-match-grid-item, .goalv-detailed-voting').each(function() {
                ...
                if (self.votingInProgress.has(`${matchId}-${location}`)) {
                    return;
                }
                ...
                .done(function(response) {
                    if (response.success && response.data) {
                        self.updateVoteResultsOnly($match, response.data, location);
                    }
                });
            });
        },
        ...
        loadMoreMatches: function() {
            ...
            .done((response) => {
                if (response.success && response.data.matches.length > 0) {
                    self.appendNewMatches(response.data.matches);
                    self.infiniteScroll.currentPage = nextPage;

                    if (response.data.matches.length < self.infiniteScroll.perPage) {
                        self.infiniteScroll.hasMoreMatches = false;
                        self.showEndMessage();
                    }
                } else {
                    self.infiniteScroll.hasMoreMatches = false;
            ...
            .always(() => {
                self.infiniteScroll.isLoading = false;
    ...
    // Legacy support - keep old reference
    window.GoalVFrontend = window.GoalVVoting;
